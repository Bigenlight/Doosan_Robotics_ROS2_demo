...
def main(args=None):
    ...
    def grip_cup(cup_index, last_pose = [366.998, 6.125, 194.183, 3.263, -179.907, 3.271]):
        ...
        decreased_height_grip = [0, 0, (-1) * CUP_STACK_GAP * cup_index, 0, 0, 0]  # 층별 높이 보정
    ...
    measure_approach = [405.801, 222.796, 260.0, 90.0, 180.0, 90.0]
    ...
    current_pose, status = get_current_posx()
    cup_starting_point_top = current_pose  # 측정된 좌표를 사용

    # 힘제어 off
    release_compliance_ctrl()

    # 측정 후 살짝 위로 복귀 (예: 20mm 위로)
    z_up_20 = [0, 0, 20, 0, 0, 0]
    measure_return = [a + b for a, b in zip(cup_starting_point_top, z_up_20)]
    movel(measure_return)
    # ------------------------------------------------------------
    # 측정 완료. 이후부터 cup_starting_point_top 사용.
    # ------------------------------------------------------------
    ...
    for z in range(3):
        ...
        for x in range(3 - z):
            x_add = [
                ((-1) * CUP_DIAMETER * (root3 / 3) * z) + 
                ((-1) * CUP_DIAMETER * (root3 / 2) * x),
                0, 0, 0, 0, 0
            ]
