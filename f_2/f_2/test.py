...
def test_sort_heights():
    """
    sort_heights 함수를 테스트하는 함수입니다.
    """
    test_data = {
        '0': 10.42,
        '1': 15.33,
    ...
    sorted_data = sort_heights(test_data)
    assert sorted_data == expected_sorted_data, f"Expected {expected_sorted_data}, but got {sorted_data}"
    print("test_sort_heights passed.")

def test_validate_heights():
    """
    validate_heights 함수를 테스트하는 함수입니다.
    """
    # 정상 케이스
    valid_data = {
        '0': 10.42,
    ...
    try:
        validate_heights(valid_data)
        print("test_validate_heights (valid data) passed.")
    except ValueError:
        print("test_validate_heights (valid data) failed.")
    ...
    try:
        validate_heights(invalid_data)
        print("test_validate_heights (invalid data) failed.")
    except ValueError as e:
        print(f"test_validate_heights (invalid data) passed with error: {e}")

def test_pick_and_place_sorted():
    """
    pick_and_place_sorted 함수를 테스트하는 함수입니다.
    실제 로봇을 제어하지 않고, 함수의 로직을 확인합니다.
    """
    sorted_data = {
        '0': 10.42,
        '6': 10.44,
    ...
    # Mocking movel and gripper functions
    def mock_movel(pose):
        print(f"Mock movel to {pose}")

    def mock_gripper_grip():
        print("Mock gripper_grip")

    def mock_gripper_release():
        print("Mock gripper_release")

    # Replace the actual functions with mocks
    global movel, gripper_grip, gripper_release
    movel_backup = movel
    gripper_grip_backup = gripper_grip
    gripper_release_backup = gripper_release

    movel = mock_movel
    gripper_grip = mock_gripper_grip
    gripper_release = mock_gripper_release

    try:
        pick_and_place_sorted(sorted_data)
        print("test_pick_and_place_sorted passed.")
    finally:
        # Restore the original functions
        movel = movel_backup
        gripper_grip = gripper_grip_backup
        gripper_release = gripper_release_backup

if __name__ == "__main__":
    # 테스트 함수 호출
    test_sort_heights()
    test_validate_heights()
    test_pick_and_place_sorted()
